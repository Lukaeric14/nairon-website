            ...
            <div
              ...
              className={`rounded-2xl p-8 ring-1 transition-all ${
                tier.highlight
                  ...
                  : "bg-[#0C0C0C]/[0.03] ring-white/10 hover:ring-white/20"
              }`}
            >
              <div className="flex items-center gap-3 mb-4">
                <h3 className="text-[#1A1916] text-xl font-medium">
                  {tier.name}
                </h3>
                {tier.highlight && (
              ...
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-[#1A1916] text-4xl font-light tracking-tight">
                  {tier.price}
                </span>
                <span className="text-[#5C584F] text-sm">{tier.period}</span>
              </div>
              {tier.annualNote && (
                <p className="text-[#5C584F] text-sm mb-4">{tier.annualNote}</p>
              )}
              {!tier.annualNote && <div className="mb-4" />}

              <p className="text-[#5C584F] text-sm mb-6">{tier.description}</p>
                  ...
                  <li key={feature} className="flex items-start gap-2.5">
                    <Check
                      className={`h-4 w-4 mt-0.5 shrink-0 ${
                        tier.highlight ? "text-[#C9A96E]" : "text-[#5C584F]"
                      }`}
                    />
                    <span className="text-[#1A1916] text-sm">{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              <Button
                ...
                className={`w-full ${
                  tier.highlight
                    ...
                    : "bg-[#0C0C0C]/10 text-[#1A1916] hover:bg-[#0C0C0C]/20"
                }`}
              >
                <a
                  href={tier.ctaHref}
