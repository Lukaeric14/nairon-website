...
export function SelectionExplainer({ slug }: { slug: string }) {
	...
	return (
		<aside className="fixed right-3 bottom-3 z-[60] w-[min(390px,calc(100vw-24px))] border border-ds-border bg-ds-surface-raised p-4 font-geist text-ds-text-primary shadow-[0_20px_70px_rgba(20,23,31,.16)]" aria-label="Explain selected article text">
			<div className="flex items-center gap-2"><Sparkles className="size-3.5 text-[var(--brand-dark)]" /><p className="text-[12px] font-semibold">Help me understand this</p><button className="ml-auto p-1 text-ds-text-tertiary hover:text-ds-text-primary" onClick={close} type="button" aria-label="Close explanation"><X className="size-4" /></button></div>
			<blockquote className="mt-3 line-clamp-3 border-l-2 border-[var(--brand-gold)] pl-3 text-[13px] leading-5 text-ds-text-secondary">“{passage}”</blockquote>
			<div className="mt-4 flex flex-wrap gap-1.5">{([['simple', 'Explain simply'], ['example', 'Give an example'], ['deeper', 'Go deeper']] as const).map(([style, label]) => <button className="border border-ds-border bg-ds-surface px-2.5 py-2 font-mono text-[8px] uppercase tracking-[0.08em] text-ds-text-secondary hover:border-ds-text-tertiary hover:text-ds-text-primary disabled:opacity-40" disabled={Boolean(pending)} key={style} onClick={() => void request(style)} type="button">{pending === style ? <Loader2 className="mr-1 inline size-3 animate-spin" /> : null}{label}</button>)}</div>
			{answer ? <div className="mt-4 border-t border-ds-border pt-4 text-[13px] leading-6 text-ds-text-secondary"><p>{answer}</p><p className="mt-3 font-mono text-[8px] uppercase tracking-[0.1em] text-ds-text-tertiary">AI support · not a new article claim</p></div> : null}
